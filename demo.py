...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        # adapter_loss = adapter_loss_list
        x = self.norm(x)
        return self.pre_logits(x[:, 0]),lora_loss,adapter_loss


    def forward(self, x):
        x,lora_loss,adapter_loss = self.forward_features(x) # 0th branch for classifier
        x = self.head(x)
        return x,lora_loss,adapter_loss
...
if __name__ == '__main__':
    ...
    model_path = '/data2/law/ViT_MOE/MoE_FFD.tar'
